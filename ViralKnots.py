...
def viral_knots(seq_filename, step, window, pk_predictors=[], pk_predict=False, bpp_packages=[], shape_data_folder=None, shape_data_sets=[], shapeknots=False,
    ...
    dfs = []
    if spawn:
        #TO DO: note that the number of jobs will be bigger than specified if using circularize function, so must fix this somehow
        temp_folder = get_random_folder(8)
        print('temp folder is ' + temp_folder)
    ...
    if pk_predict:
        if spawn:
            ...
            bpp_packages_str = " ".join(bpp_packages)
            ...
            for name in pk_predictors:
                struct_list = []
                if name == 'threshknot':
                    for bpp_package in bpp_packages:
                    ...
                    for seq,coord in zip(seq_windows,coords):
                        struct_list.append(get_structure(seq, coord, pk_predictor=name, window=window, bpp_package=None,linear_partition=linear_partition))
    ...
    if shapeknots:
        if spawn:
            ...
            for idx, shape_set in enumerate(all_shape_windows):
                for i in range(0,len(seq_windows),size_job):
                    seq_windows_str = " ".join([str(x) for x in seq_windows[i:i+size_job]])
                    coords_str = " ".join([str(x) for x in coords[i:i+size_job]])
    ...
    if spawn:
        ...
        while 2*(pk_predict_jobs+shape_jobs) > len(os.listdir(temp_folder)):
    ...
    for i,coord in enumerate(coords):
        ...
        df2 = df1.loc[df1['start'] == str(coord)].copy() # RCK to get rid of warning, since its a copy now also got rid of the .loc fix below (x8)
...
if __name__=='__main__':
    ...
    #add a check that if running threshknot, need to specify bpp package
    ...
    for package in args.pk_predictors:
        if package == "threshknot":
            if args.linear_partition:
                #assert args.bpp_packages in ['vienna_2', 'contrafold_2', 'eternafold','contrafold','vienna'], "LinearPartition only implemented for vienna_2, contrafold_2, eternafold, vienna, contrafold."
                package_str += f' threshknot with base-pair-probaility matrix from {args.bpp_packages} with linear partition,'
            else:
                package_str += f' threshknot with base-pair-probaility matrix from {args.bpp_packages},'
        else:
